namespace App.ModifierTemplates.Controllers {
	...
	interface IModifierTemplatesScope extends ng.IScope {
		filterTimeout: ng.IPromise<void>;
	}

	class ModifierTemplatesCtrl {
		...
		static $inject = ['$scope', '$timeout', '$location', 'modifierTemplatesService', 'userSettingsService'];
		constructor(
			private $scope: IModifierTemplatesScope,
		...
		filterChanged(): void {
			...
			this.$scope.filterTimeout = this.$timeout(() => {
				this.getFilteredgetModifierTemplates();
		...
		getFilteredgetModifierTemplates(): void {
			this.searchOptions.filterText = "";
			$.each(this.gridApi.grid.columns,
				(index, column) => {
